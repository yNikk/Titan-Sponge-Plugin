...
public class TitanPlayerCommand implements CommandExecutor {
            ...
            if (cmd.getOne("teamname").isPresent() && cmd.getOne("playername1").isPresent()) {
                ...
                if (tm.teamExists(teamname)) {
                    if (Data.getPlayerFromString(playername) != null) {
                        tm.getTeam(teamname).addPlayer(Data.getPlayerFromString(playername));
                        Chat.send(p, "Der Spieler wurde hinzugef\u00fcgt!", Chat.SUCCESS);
                    } else {
